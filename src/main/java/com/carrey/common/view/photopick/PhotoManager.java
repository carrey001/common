...
public class PhotoManager {
    ...
    private String processPhoto(String filePath, int w, int h) {
        Bitmap bitmap = null;
        try {
            bitmap = ImageDecoder.decodeBitmap(new File(filePath), new ImageOptions.Builder().setSize(w, h).build(), null);
//        Bitmap bitmap = ImageDecoder.decodeSampledBitmapFromFile(filePath, new BitmapSize(w, h), null);
        } catch (IOException e) {
            e.printStackTrace();
        }
        if (bitmap == null) {
